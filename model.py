...
class Generator(BaseNet):
    ...
    def __init__(self, num_latent_dims, num_img_channels, max_num_filters=64, device=torch.device("cpu")):
        ...
        # we assume input shape of (num_latent_dims, 1, 1)
        ...
        self.conv3 = nn.ConvTranspose2d(num_filters_2,   num_filters_3,    kernel_size=4, 
            stride=2, padding=1, bias=False)
        # Output num_filters_4 x 28 x 28
        self.conv4 = nn.ConvTranspose2d(num_filters_3,   num_img_channels, kernel_size=4, 
            stride=2, padding=1, bias=False)
        ...
        # Upscaling of the latent vector from two input dimensions
      #  self.lat_lin = nn.Linear(num_latent_dims, num_latent_dims*8)
