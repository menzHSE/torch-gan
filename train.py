...
def init_weights(model):
    mu = 0.0
    sigma = 0.02
    if isinstance(model, torch.nn.Conv2d) or isinstance(
...
def train(
    ...
    # over time.
    num_images = 64
    fixed_latent_vectors = utils.sample_latent_vectors(
        num_images, G.num_latent_dims, dev
    )
    ...
    for epoch in range(num_epochs):
        ...
        (
            ...
            throughput_list,
            batch_count,
        ) = train_epoch(dev, epoch, G, D, train_loader, optimizer_G, optimizer_D)
